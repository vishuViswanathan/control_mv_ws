#define VERSIONMSG "stepperControlSP01.ino"
...
ISR (SPI_STC_vect) {
  c = SPDR;
  // Serial.print("In ISR:\t");Serial.println(char(c));
  if (receivePos == 0) 
    prepareSendData();
  if (!spiReceived) {
...
void prepareSendData() {
  if (!sendDataReady) {
    ...
    sprintf(sendBuf, "%+011d %+011d", l_motor_steps, r_motor_steps);
...
int runCommand(char *msg) {
  ...
  switch(msg[0]) {   
    case RESET_ENCODERS:
      ...
      break;
    case MOTOR_SPEEDS:
      Serial.println("Got Speed setting");
      /* Reset the auto stop timer */
      lastMotorCommand = millis();
      retVal =  (sscanf(msg, "Speed l:+5d r:+5d", &arg1, &arg2) == 2);
      if (retVal) {
          Serial.println("Got Speed setting");
